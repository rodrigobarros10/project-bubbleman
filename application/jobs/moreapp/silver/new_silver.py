...
# --- 1. FUNÇÕES AUXILIARES E DE LEITURA (MELHORADAS) ---
...
def corrigir_caracteres_definitiva(texto: str) -> str:
    ...
    # Estágio 1: Tenta a correção de dupla codificação (o problema exato de "CAMINHÃƒO")
    try:
        texto_corrigido = texto.encode('latin-1').decode('utf-8')
        # Se a correção funcionou, usa o resultado. Se não, continua com o texto original.
        # Isso evita que textos já corretos sejam danificados.
        if "Ã" not in texto_corrigido:
            texto = texto_corrigido
    except (UnicodeEncodeError, UnicodeDecodeError):
        pass # Ignora e continua se a conversão não for possível

    # Estágio 2: Usa a biblioteca ftfy, que é excelente para resolver bagunças de texto
...
def ler_csv_da_bronze(caminho_local: str) -> pd.DataFrame:
    ...
    for encoding in encodings_para_tentar:
        try:
            # CORREÇÃO: Usar a variável 'encoding' do loop
            df = pd.read_csv(
                caminho_local,
                delimiter='|',
                dtype=str,
...
# --- 2. FUNÇÕES DE TRANSFORMAÇÃO (UMA PARA CADA TIPO DE FORMULÁRIO) ---
...
def _transform_ilu_produtividade(df: pd.DataFrame) -> dict:
    ...
    # CORREÇÃO: Adicionar pd.to_numeric para evitar FutureWarning
    ...
    # A função pode retornar múltiplos DataFrames ou outros objetos
    # A chave "Metadados_Produtividade" contém um dicionário, não um DataFrame
    metadados = {
...
def roteador_de_transformacao(df_bronze: pd.DataFrame, nome_arquivo: str) -> dict:
    ...
    # MELHORIA: Adicionar espaço no final para evitar correspondências parciais
...
# --- 4. ORQUESTRADOR DO PROCESSO (BRONZE -> SILVER) ---
	...
	try:
		...
		for file_key in csv_files:
			...
			try:
				...
				# CORREÇÃO: Iterar sobre os itens do dicionário (chave, valor)
				# Isso resolve o erro 'dict' object has no attribute 'empty'
				for nome_tabela, df_ou_obj in tabelas_silver.items():
					# Processa apenas se o valor for um DataFrame do pandas
					if isinstance(df_ou_obj, pd.DataFrame):
						df_silver = df_ou_obj  # Renomeia para clareza
...
if __name__ == "__main__":
	# Supondo que as credenciais da AWS estão configuradas como variáveis de ambiente
	...
	SILVER_PREFIXO = "project-bubbleman/moreapp/submissions/"  # Prefixo melhorado para Silver
